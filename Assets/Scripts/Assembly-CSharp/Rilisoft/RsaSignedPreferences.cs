...
namespace Rilisoft.PixlGun3D
{
	internal sealed class RsaSignedPreferences : SignedPreferences
	{
		...
		private const string Prefix = "com.Fuckhead.PixlGun3D";
		...
		private readonly byte[] _prefixBytes = Encoding.UTF8.GetBytes("com.Fuckhead.PixlGun3D");
		...
		{
			if (key.StartsWith("com.Fuckhead.PixlGun3D"))
			{
		...
		private string GetSignatureKey(string key)
		{
			...
			return string.Format("{0}_{1}", "com.Fuckhead.PixlGun3D", Convert.ToBase64String(inArray));
		}
